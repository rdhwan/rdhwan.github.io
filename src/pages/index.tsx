    ...
    <>
      ...
      <div className="flex flex-col py-6 font-overpass text-secondary">
        ...
        <p className="text-lg text-justify py-4">
          ...
          <a href="https://tk.umn.ac.id" target="_blank" className="underline">
          ...
          ideal candidate for any organization seeking a talented and innovative
          programmer. I am eager to take on any challenging projects that comes
          my way and am confident in my ability to deliver exceptional results.
          Please do not hesitate to reach out to me if you believe I can be of
            ...
            {frontEnd.map((e) => (
              <a href={e.ghLink} target="_blank">
                <img
                  src={e.logoPath}
                  alt={e.framework}
                  className="h-16 hover:opacity-50 transition-all duration-100"
                />
              </a>
            ))}
          </div>
            ...
            {backEnd.map((e) => (
              <a href={e.ghLink} target="_blank">
                <img
                  src={e.logoPath}
                  alt={e.framework}
                  className="h-16 hover:opacity-50 transition-all duration-150 delay-75"
                />
              </a>
            ))}
          </div>
      ...
      <div className="flex w-full flex-col py-4 text-secondary">
        ...
        <div className="flex flex-1 flex-col py-4 px-2">
          {works.map((e) => (
            <Link
              to={`/works/${e.slug}`}
              className="flex flex-col w-full my-4 hover:opacity-50 transition-all duration-150 delay-75"
            >
              <img
                src={e.images[0]}
                alt={e.title}
                className="h-72 w-full object-top object-cover rounded-xl"
              />

              <div className="flex flex-col items-center justify-center">
                <h1 className="text-2xl font-bold font-montserrat pt-4">
                  {e.title}
                </h1>
                <p className="text-lg text-justify">{e.short}</p>
              </div>
            </Link>
          ))}
        </div>
